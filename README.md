...
- `assets/icons/` — the reusable icon set (17 SVGs, one consistent line style; recolor by changing `stroke="currentColor"` or wrapping in an element with a CSS `color`)
...
2. **Photos**: replace the three files in `assets/img/` (hero 1920×1080, mentor 900×1000, community 1000×900). Search `PLACEHOLDER:` in index.html to find each spot. Keep the `width`/`height` attributes and `loading="lazy"` on the two below-the-fold photos.
...
## Logo files
- `assets/img/logo-badge.png` — the real badge, background removed, transparent, 900px wide (cut from your 2000×2000 original)
- `favicon.png` — 128px favicon from the badge
- `assets/img/logo-wordmark.png` / `logo-full.png` — older low-res lockup cuts; superseded, kept only in case you need the horizontal text version
